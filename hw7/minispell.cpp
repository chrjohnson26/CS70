...
void usage(const char* progname) {
    std::cerr << "Usage: " << progname << " [options] [file-to-check ...]\n"
              ...
              << "  -f, --file-order       Insert words in the order they "
                 "appear (default).\n"
              << "  -s, --shuffled-order   Insert words in random order,"
                 " by shuffling the input.\n"
              << "  -b, --balanced-order   Insert words in a balanced order\n"
              << "  -r, --random-insert    Use a tree with RANDOMIZED "
                 "insertion style\n"
              << "  -t, --root-insert      Use a tree with ROOT insertion "
                 "style\n"
              << "  -l, --leaf-insert      Use a tree with LEAF insertion "
                 "style (default)\n"
...
int main(int argc, const char** argv) {
    ...
    while (!args.empty() && args.front()[0] == '-') {
        ...
        if (option == "-f" || option == "--file-order") {
        ...
        } else if (option == "-t" || option == "--root-insert") {
            insertScheme = ROOT;
        } else if (option == "-l" || option == "--leaf-insert") {
            insertScheme = LEAF;
        } else if (option == "-r" || option == "--random-insert") {
            ...
            bool isDashN = option == "-n" || option == "--num-dict-words";
            args.pop_front();
            if (args.empty()) {
